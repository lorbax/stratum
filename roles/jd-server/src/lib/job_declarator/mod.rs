...
//impl AddTrasactionsToMempool {
//    fn send_to_mempool(&self) -> async_channel::Send<'static, AddTrasactionsToMempoolInner> {
//        self.sender_add_txs_to_mempool.send(self.add_txs_to_mempool_inner.clone())
//    }
//}
...
impl JobDeclaratorDownstream {
    pub fn new(
        ...
        let tokens = Id::new();
        let add_txs_to_mempool_inner = AddTrasactionsToMempoolInner { known_transactions: vec![], unknown_transactions: vec![] };
        ...
        Self {
            ...
            add_txs_to_mempool: AddTrasactionsToMempool { add_txs_to_mempool_inner, sender_add_txs_to_mempool},
        }
    }
    ...
    fn get_block_hex(
        ...
        let (last_declare_, _, _) = self_mutex.clone()
    ...
    fn are_all_job_transactions_present(
        ...
        let (_, transactions_with_state, _) = self_mutex.clone()
            .safe_lock(|x| x.declared_mining_job.clone())
            .map_err(|e| Box::new(JdsError::PoisonLock(e.to_string())))?;
    ...
    async fn send_txs_to_mempool(self_mutex: Arc<Mutex<Self>>) {
        let add_txs_to_mempool = self_mutex.safe_lock(|a| a.add_txs_to_mempool.clone()).unwrap(); 
        ...
        let _= sender_add_txs_to_mempool.send(add_txs_to_mempool_inner).await;
        // the trasnactions sent to the mempool can be freed
        let _ = self_mutex.safe_lock(|a| {
            a.add_txs_to_mempool.add_txs_to_mempool_inner = AddTrasactionsToMempoolInner { known_transactions: vec![], unknown_transactions: vec![] };
        });
    }
                ...
                match recv.recv().await {
                    Ok(message) => {
                        ...
                        match next_message_to_send {
                            Ok(SendTo::Respond(m)) => {
                                match m {
                                    JobDeclaration::AllocateMiningJobToken(_) => info!("Received AMJT"),
                                    JobDeclaration::AllocateMiningJobTokenSuccess(_) => error!("Unexpected message: AMJTS"),
                                    JobDeclaration::DeclareMiningJob(_) => {
                                        info!("Received DMJ");
                                        Self::send_txs_to_mempool(self_mutex.clone()).await;
                                    },
                                    JobDeclaration::DeclareMiningJobError(_) => error!("Unexpected message: DMJE"),
                                    JobDeclaration::DeclareMiningJobSuccess(_) => error!("Unexpected message: DMJS"),
                                    JobDeclaration::IdentifyTransactions(_) => error!("Unexpected message: IT"),
                                    JobDeclaration::IdentifyTransactionsSuccess(_) => error!("Unexpected message: ITS"),
                                    JobDeclaration::ProvideMissingTransactions(_) => error!("Unexpected message: PMT"),
                            ...
                            Ok(SendTo::None(m)) => {
                                match m {
                                    Some(JobDeclaration::SubmitSolution(message)) => {
                                        match Self::are_all_job_transactions_present(self_mutex.clone()) {
                                            Ok(_) => {
                                            ...
                                            Err(error) => {
                                                ...
                                                let retrieve_transactions = AddTrasactionsToMempoolInner {
                                                    known_transactions,
                                                    unknown_transactions: Vec::new(),
                                                };
                                                let mempool = self_mutex.clone().safe_lock(|a|a.mempool.clone()).unwrap();
